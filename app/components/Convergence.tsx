...
function ConvergenceItem({
  ...
  // Angle toward center (for stretch direction)
  const angle = Math.atan2(-tool.y, -tool.x) * (180 / Math.PI);

  // Position: scatter → converge to center with acceleration
  const x = useTransform(
    scrollYProgress,
    [0.08, 0.18, 0.30, 0.42, 0.50],
    [tool.x, tool.x, tool.x * 0.6, tool.x * 0.1, 0]
  );
  const y = useTransform(
    scrollYProgress,
    [0.08, 0.18, 0.30, 0.42, 0.50],
    [tool.y, tool.y, tool.y * 0.6, tool.y * 0.1, 0]
  );

  // Rotation: initial → align toward center as it accelerates
  const rotate = useTransform(
    scrollYProgress,
    [0.18, 0.32, 0.45],
    [tool.rotate, tool.rotate * 0.5, angle]
  );

  // Stretch distortion — the "motion streak" effect
  ...
  // Opacity: fade in → hold → dissolve
  const opacity = useTransform(
    scrollYProgress,
    [0.05, 0.15, 0.38, 0.50],
    [0, tool.opacity, tool.opacity * 0.5, 0]
  );

  // Blur increases as it accelerates
...
export default function Convergence() {
  ...
  // OLI emergence from singularity
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        <motion.div
          ...
          style={{
            ...
            background:
              "radial-gradient(circle 200px at 50% 50%, rgba(255,255,255,0.9), transparent 70%)",
          }}
        />
